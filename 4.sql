...
CREATE DATABASE IF NOT EXISTS sem4;
USE sem4;
...
WHERE name = "Murzik") AS myfile
ON myfile.shops_id = shops.id;
...
FROM analysis
JOIN (Select groupsan.*, Orders.*
FROM groupsan jOIN Orders
ON groupsan.gr_id = Orders.ord_an) as temp
ON analysis.an_group = temp.gr_id
WHERE (temp.ord_datetime BETWEEN '2020-02-05' AND '2020-02-12')
ORDER BY temp.ord_datetime;
